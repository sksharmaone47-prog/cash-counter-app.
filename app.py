...
# Updated Notes List (Removed 5, 2, 1)
...
# --- ALIGNED INPUT SECTION (2000 x 5 = 10000 Format) ---
for n in notes:
    col1, col2, col3 = st.columns([1.2, 2.3, 3.5])
    with col1:
        st.write(f"**₹{n}**")
    with col2:
        # Input box to change the quantity
        count = st.number_input(f"x {n}", min_value=0, step=1, value=0, key=f"q_{n}", label_visibility="collapsed")
        counts[n] = count
    with col3:
        subtotal = n * count
        totals.append(subtotal)
        # Format: 2000 x 5 = 10000 (Aligned vertically)
        st.markdown(f'<p class="calc-row"> {n:>4} x {count:<3} = {subtotal:>8}</p>', unsafe_allow_html=True)

# Coins Section (Directly after 10)
st.divider()
c_col1, c_col2, c_col3 = st.columns([1.2, 2.3, 3.5])
with c_col1:
    st.write("**Coins**")
with col2:
    coin_val = st.number_input("Coins Value", min_value=0, step=1, value=0, key="coin_v", label_visibility="collapsed")
with col3:
...
try:
    # Proper Spacing and removed 'And'
